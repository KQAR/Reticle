...
1. **Screenshot degrade surfaces (`SurfaceView` / `FLAG_SECURE`).** Known degrade
...
2. **Third-party WebView kernels (X5/UC) — document, do not build.** `WebViewBridge`
...
3. **Structural boundaries, written down as boundaries.** Closed shadow roots,
