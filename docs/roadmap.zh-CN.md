...
E2E 跑得久，而这两个已知泄漏恰好只在长跑时咬人。

- **`network-bodies/` 无淘汰。** 每条 flow 写一个 body 产物且从不回收，长校验会话会
  漏磁盘。淘汰必须与事件环的淘汰联动：body 是仍被存活事件引用的证据，不能在其脚下抽走。
- **`awaitRuntime` 用固定次数 × sleep**，不是墙钟 deadline（`HelperRuntime.kt`）——
  与 `Injector.connectWithHandshake`、`HelperVerify.pollForChange` 不一致，探针无响应
  时会超出预算。
...
输出量级，不给等级。顺序 **A1 → A2 → A3**（A4 已作为 `replay gif` 落地）。
...
- **A2 — 视觉回归（`reticle diff visual`）。** 两个构建之间做像素 diff，输出变化区域
  ...
  漂了"。阈值是提示，不是判定。
- **A3 — 设计还原度证据（`reticle diff design`）。** 把设计稿组件框与实时节点 rect
  对齐，输出每个区域的位置/尺寸/颜色/文案偏差与叠加图；设计数据走现有 Figma 通道。
  不打分——打分是消费方的事。
