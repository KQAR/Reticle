...
| A screen covered by another | `occludedBy` now also considers a **later-drawn sibling in the same window**, not just a higher window — the shape a hybrid app really has when a second screen is pushed over a still-alive one. The cover must be interactive, which is the honest limit: a touch falls through a view that does not consume it | A **COVERED BY ANOTHER** e2e section over both variants of the nested-WebView scenario: the covered control is marked, the covering page's own control is not, and the inset variant guards the false positive by tapping a genuinely visible control that must still work |
