...
| ~~**Which page is in front is not stated**~~ **(fixed)** | Two `ViewPager` pages, and separately two live `WebView`s, are flattened into one projection ordered by y — a foreground node and a background node can be adjacent lines with nothing distinguishing them. `--window top` does not help: they share one window | Mark background-page / background-WebView subtrees (`offscreen`, `inactive`), or let a `--window`-style scope narrow to the active page within a window |
...
| ~~**Off-screen DOM nodes count as visible for `--label`**~~ **(fixed)** | An animated numeric counter contributes ~40 `li` nodes, most clipped out of the viewport by an ancestor's overflow (frames at negative y, or far below the screen). Selecting a numeric option elsewhere on the page failed with `label '5' matched 4 visible nodes` — three of them clipped counter digits | Exclude nodes clipped out of their scroll container from `--label` candidacy, and from `ui outline` numbering |
| ~~**`ui outline` numbers the whole scrollable tree**~~ **(fixed)** | On a 1080x2412 screen, `outline` emitted 135 aliases, the last at `y=10800` — roughly 15 were actually on screen. The skill documentation describes it as printing *visible* labelled/interactive nodes | Restrict to the viewport by default; offer `--all` for the current behaviour. This also makes `@N` numbering stable enough to act on |

## D. Reporting gaps

| Gap | What was measured | Fix shape |
| --- | --- | --- |
| ~~**`--verify` reads like a failure after a navigation**~~ **(fixed)** | `act tap --resource-id <id> --verify` printed `verify @<id>: no change` while the same action's trace recorded `101 change(s)` — the screen had been replaced entirely | Say what was compared. When the target node is gone or the screen changed wholesale, report that instead of `no change` |
| ~~**`--point` is silent about why it was needed**~~ **(fixed)** | 23 of ~50 taps were `--point`. Nothing in any of those results notes that no selector was available, or why | Have `act … --point` emit `warning: no semantic selector covers this region` with the reason (cross-origin iframe / no interactive node captured / DOM subtree unreadable). The warning is what converts a silent degradation into a filed gap |
| ~~**There is no way to ask "how much of this screen is unreachable?"**~~ **(fixed)** | The only way to discover the gaps above was to run a real flow and notice the fallbacks by hand | Add a coverage check (`ui coverage`) reporting the share of visible, interactive-looking screen area with no addressable node over it. That number is the direct measure of the blind-agent contract |
...
| --- | --- | --- |
| The fallback reports itself | Every `act tap --point` carries a coverage verdict: either `no semantic selector covers (x,y) — <reason>` naming what is in the way, or `--point was not needed at (x,y)` naming the flag that would have worked — the quieter loss, since a coordinate throws away the re-resolve, the settle confirm and the stale-rect evidence. `ui coverage` asks the same question about the whole screen, sampled on a stated grid, listing each unreachable region by reason / host ref / rect. Two rules the first cut got wrong and a device corrected: a **screen-sized tappable container** is not cover for the points inside it (an Android `WebView` is clickable and carries an id, which reported a real hybrid screen as 100% addressable), and only the **top window layer** at a point may answer for it (the home list behind a scenario screen was smaller, so a smallest-node rule reached through the front screen and named `--test-id scenario.list` for a coordinate inside a cross-origin frame) | `reticle-protocol/fixtures/screen-coverage.cases.json` — four cases over both platforms, pinning every verdict and the exact report lines — plus `ScreenCoverageContractTest` / `ScreenCoverageContractTests` and `PointCoverageOutcomeTests` (the verdict stays out of the flat fields line and survives `--json`); a **COVERAGE** e2e section in both `scripts/e2e-android.sh` (the cross-origin frame appears as a named gap; a coordinate inside it is justified; one over `complex.iframeButton` is not; a selector tap warns about nothing) and `scripts/e2e-ios.sh` |
| Degradation that names itself | `iframe:cross-origin` on a frame whose document browser policy withholds — the absence was already a documented boundary, the silence was the defect; `--verify` says what it compared instead of "no change" beside a trace of 101 changes; `reticle --help` lists the commands and their subcommands, and `reticle app` with no subcommand names the ones that exist | A **CROSS-ORIGIN FRAME** e2e section running both frame twins on one screen — a `data:` URL frame has an opaque origin, which is what finally makes this testable offline (`docs/boundaries.md` had it recorded as "not exercised") |
| A screen covered by another | `occludedBy` now also considers a **later-drawn sibling in the same window**, not just a higher window — the shape a hybrid app really has when a second screen is pushed over a still-alive one. The cover must CONSUME the touch, and native and DOM covers have opposite defaults: a native view has to be interactive (a touch falls through one that does not consume it), while a DOM element eats the click wherever its box lies unless the page said `pointer-events: none` — so an in-page sheet's backdrop, which publishes no role, tabindex or handler, now counts | A **COVERED BY ANOTHER** e2e section over both variants of the nested-WebView scenario: the covered control is marked, the covering page's own control is not, and the inset variant guards the false positive by tapping a genuinely visible control that must still work |
| Clipping and caps | A node laid out entirely outside a clipping ancestor's box is not visible (`compact` omits it, the semantic tree keeps it, `custom.domClipped` names why) — `position: fixed` escapes the rule, as CSS says, and a partially visible node stays visible; `ui outline` numbers what is on screen rather than the whole scrollable tree; a DOM walk that stops at the traversal's node cap says `dom:capped(N)` on its host, which the projection cap already did and this one did not | `FormSemanticsTest` (the cap marker, and that a completed walk does not read as a cap of zero); a **CLIPPED NODES** e2e section over a new `clipped` fixture — an `overflow:hidden` roller, a scroll port, and 420 tiled cells to trip the cap |
...
That reporting now exists, which changes what an open row in this file looks like.
A coordinate tap prints a verdict and `ui coverage` prints the screen's share, so a
future gap does not have to be discovered by driving a flow and counting by hand:
it arrives as a warning naming a reason, or as an unreachable region in a report.
The rule above is unchanged; what changed is that Reticle, not the reader, is the
one keeping score.
