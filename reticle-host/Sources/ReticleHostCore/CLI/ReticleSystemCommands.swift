...
enum ReticleSystemCommands {
    ...
    static func dispatch(_ args: Args) async throws {
        ...
        switch sub {
        case "prepare": try await prepare(args)
        case "status": try await status(args)
        case "stop": try await stop(args)
        case "overlay": try await overlay(args)
        case "tree": try await tree(args)
        case "tap": try await tap(args)
        case "screenshot": try await screenshot(args)
        case "home": try await home(args)
        case "activate": try await activate(args)
        default:
    ...
    static func prepare(_ args: Args) async throws {
        guard let team = args.option("team") else {
            let teams = await describeTeams()
            throw HelperError(
        ...
        let backend = try await makeBackend(args)
        let outcome = try await backend.prepare(team: team, runnerProjectPath: runnerProjectPath())
    ...
    static func status(_ args: Args) async throws {
        let backend = try await makeBackend(args)
        let s = await backend.status()
        print(s.describe)
        if let advice = s.advice { print("  \(advice)") }
    }

    static func stop(_ args: Args) async throws {
        let backend = try await makeBackend(args)
    ...
    static func overlay(_ args: Args) async throws {
        let backend = try await makeBackend(args)
        render(try await backend.overlay(), args: args)
    }

    static func tree(_ args: Args) async throws {
        ...
        let backend = try await makeBackend(args)
        render(try await backend.tree(target: target), args: args)
    }
    ...
    static func screenshot(_ args: Args) async throws {
        let backend = try await makeBackend(args)
    ...
    static func tap(_ args: Args) async throws {
        let backend = try await makeBackend(args)
        if let label = args.option("label") {
            print(try await backend.act { try $0.tap(label: label) }.describe)
            return
        }
        ...
        print(try await backend.act { try $0.tap(x: x, y: y) }.describe)
    }

    static func home(_ args: Args) async throws {
        let backend = try await makeBackend(args)
        print(try await backend.act { try $0.home() }.describe)
    }

    static func activate(_ args: Args) async throws {
        ...
        let backend = try await makeBackend(args)
        print(try await backend.act { try $0.activate(bundleId: bundleId) }.describe)
    }

    // MARK: - Wiring

    static func makeBackend(_ args: Args) async throws -> IosSystemBackend {
        ...
        return try await IosSystemBackend.make(udid: udid, appBundleId: appBundleId)
    }
    ...
    static func describeTeams() async -> String {
        let teams = await IosRunnerLifecycle.usableTeams().sorted()
