...
/// and is embedded here and in `dev.reticle.core.WebViewDomScript` verbatim. It used
/// to be hand-copied between the two agents under a "KEEP IN SYNC" comment — with
/// different escaping on each side, so the copies could not even be diffed
/// mechanically. `WebViewDomScriptTests` here and `WebViewDomScriptTest` in
/// reticle-core each assert their embedded copy equals that file, so an edit to one
/// embedding, or to the file, fails the build until all three agree.
///
/// Embedded rather than loaded from a resource bundle so the injected iOS agent and
/// the Android payload dex — neither of which carries resources — behave the same as
/// the linked build.
///
...
/// pierced elements carry a chained selector (`#host >>> #inner`, resolvable by
/// `WebActivationScript`) and iframe content coordinates are offset into page space.
public enum WebViewDomScript {
