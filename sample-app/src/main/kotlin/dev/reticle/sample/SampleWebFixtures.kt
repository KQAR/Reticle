...
object SampleWebFixtures {
    ...
    /**
     * Every frame shape that has its own WALL, on one small page.
     *
     ...
     * below the viewport — so the frames under test were exactly the ones dropped,
     * while the frames that CAN be walked survived on the strength of their content.
     * Measured on an emulator: `#sandbox-frame` and its reload button were simply
     * absent from a 32-node capture of the complex page. A page whose fixtures compete
     * for the fold tests the fold, not the fixtures.
     */
    fun framesFixture(heightPx: Int): Fixture =
        Fixture(
            heightPx = heightPx,
            baseUrl = "https://reticle.dev/sample/frames",
            html = framesHtml,
        )

    val framesHtml: String = """
        <!doctype html>
        <html>
          <head>
