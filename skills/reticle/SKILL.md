...
| `iframe:cross-origin` | This frame's document is another origin's; browser policy withholds it from the page itself | Coordinates inside the frame's rect are the only path in. `--css` into it never resolves |
